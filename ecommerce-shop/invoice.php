...
$dynamic_title="Invoice | Stylishvalley.com";
include('header.php');
include('single_nav.php');

//$_SESSION['InvoiceGenerate'] = true;

if(!isset($_SESSION['InvoiceGenerate']) AND $_SESSION['InvoiceGenerate'] == false){
    echo reloader('/',0);
	exit();
	die();
}else{
    //$_SESSION["ExpiryInvoice"] = DateTime.UtcNow.AddMinutes(5);
}

/*if ($_SESSION["ExpiryInvoice"] != null && DateTime.Parse($_SESSION["ExpiryInvoice"]) < DateTime.UtcNow)
{
    $_SESSION["InvoiceGenerate"] = null;
    $_SESSION["ExpiryInvoice"] = null;
    echo reloader('/',0);
	exit();
	die();
}*/
...
$sql="SELECT * from orders WHERE customer_id = '$eb_user_id' ORDER BY id DESC LIMIT 1";
...
if(isset($_POST['submit'])){
    $_SESSION['InvoiceGenerate'] = false;
	echo reloader('/',0);
	exit();
	die();
}
