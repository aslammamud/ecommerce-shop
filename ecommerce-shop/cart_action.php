<?php
    ...
    if(isset($_SESSION['CurrentCartSession']) AND $_SESSION['CurrentCartSession'] != null){
		  ...
		  $search_product_query =  "SELECT * FROM cart WHERE cart_session = '$cart_session' AND product_id = '$pid'";
          ...
          $rowdata_fetch = mysqli_fetch_assoc($result_s_p_q);
			...
			if($rows_s_p_q > 0){
			    ...
			    $updated_qty = (int)$rowdata_fetch['qty'] +1;
			    $updated_total_price = (int)$pprice * (int)$updated_qty;
			    $item_update_query = "UPDATE cart SET qty='$updated_qty', total_price='$updated_total_price' WHERE product_id = '$pid' AND cart_session = '$cart_session'";
		    	...
		    	$insert_product_query = "INSERT INTO cart (cart_session,product_id,product_name,product_price,product_image,qty,total_price,product_code) VALUES ('$cart_session','$pid','$pname','$pprice','$pimage','$pqty','$total_price','$pcode')";
	...
	if (isset($_POST['qty'])) {
	  ...
	  $tprice = $qty * $pprice;
	  $item_update_query = "UPDATE cart SET qty='$qty', total_price='$tprice' WHERE id='$pid'";
	  $result_item_update = mysqli_query($con, $item_update_query);
	}
